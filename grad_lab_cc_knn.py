...
# Answer: I think the threshold would give better accuracy if it decreases, as k determines the # of points to average out
...
# (correctly non-HBCU), 0 false negatives (predicted hbcu but actually non-hbcu), ad 4 true positives 
# (correctly HBCU), and 15 false negatives (actual HBCU but predicted non-HBCU). Therefore, the model does not
#  identify hbcu's incredibly well, but, does do an okay job, correctly identifying 4/19 at a 2.5% prevalence rate.
...
best = None  
# we'll store the best row here

print("\nTUNING RESULTS (evaluated on TUNE set, not test):")

for k in k_list:
    for t in t_list:
        results_df, cm, acc = run_knn(X_train, y_train, X_tune, y_tune, k=k, threshold=t)
...
print("\nBEST ON TUNE (by recall, then accuracy):")
print(best)
# %%
...
print("\nFINAL TEST RESULTS (chosen using TUNE):")
print("Best k and threshold:", best_k, best_t)
print("Confusion matrix:\n", best_cm)
...
#%%
# Step 8: new target variable (flagship). WITH TUNING
...
print("\nFLAGSHIP TUNING RESULTS (evaluated on TUNE set, not test):")
...
print("\nBEST FLAGSHIP ON TUNE:")
print(best)

# FINAL TEST RESULTS (one-time evaluation)
final_results, final_cm, final_acc = run_knn(X_train, y_train, X_test, y_test, k=best_k, threshold=best_t)

print("\nFLAGSHIP FINAL TEST RESULTS (chosen using TUNE):")
print("Best k and threshold:", best_k, best_t)
print("Confusion matrix:\n", final_cm)
